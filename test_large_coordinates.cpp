...
TEST_F(LargePositionTest, EqualityOperators) {
    ...
    EXPECT_EQ(pos_c, pos_d);  // Should be equal despite different representation
    ...
    EXPECT_NE(pos_far1, pos_far2);  // Should be unequal due to distance
}

TEST_F(LargePositionTest, CoordinateConversionAccuracy) {
    ...
    float3 original_world = original.to_float3(int3(0, 0, 0));
    float3 reconstructed_world = reconstructed.to_float3(int3(0, 0, 0));
    ...
    EXPECT_NEAR(original_world.x, reconstructed_world.x, tolerance);
    EXPECT_NEAR(original_world.y, reconstructed_world.y, tolerance);
    EXPECT_NEAR(original_world.z, reconstructed_world.z, tolerance);
}

TEST_F(LargePositionTest, RelativePositioning) {
...
TEST_F(LargePositionTest, MovementSimulation) {
    ...
    float3 original_world = object_pos.to_float3(int3(0, 0, 0));
    float3 moved_world = moved_pos.to_float3(int3(0, 0, 0));
    
    float distance_moved = moved_world.x - original_world.x;
    EXPECT_NEAR(distance_moved, 5000.0f, 1e-3f);
    
    // Y and Z should remain unchanged
    EXPECT_NEAR(moved_world.y, original_world.y, 1e-3f);
    EXPECT_NEAR(moved_world.z, original_world.z, 1e-3f);
}

TEST_F(LargePositionTest, ToFloat3BoundsAssertion) {
...
TEST_F(LargePositionTest, WorldCoordinateConstructor) {
    ...
    EXPECT_NEAR(pos1.local.x, 1000.0f, 1e-5f);        // 1000 - 0*2048 = 1000
    EXPECT_NEAR(pos1.local.y, 2000.0f - 2048.0f, 1e-5f); // 2000 - 1*2048 = -48
    EXPECT_NEAR(pos1.local.z, 3000.0f - 2048.0f, 1e-5f); // 3000 - 1*2048 = 952
    
    // Test coordinates that cross cell boundaries
    LargePosition pos2(double3(2500.0, 0.0, 0.0));
    // round(2500/2048) = round(1.22) = 1, round(0/2048) = 0
    EXPECT_EQ(pos2.global, int3(1, 0, 0)); 
    EXPECT_NEAR(pos2.local.x, 2500.0f - 2048.0f, 1e-5f); // 2500 - 1*2048 = 452
    EXPECT_NEAR(pos2.local.y, 0.0f, 1e-5f);               // 0 - 0*2048 = 0
    EXPECT_NEAR(pos2.local.z, 0.0f, 1e-5f);               // 0 - 0*2048 = 0
    
    // Test negative coordinates
    LargePosition pos3(double3(-500.0, -1000.0, -2500.0));
    // round(-500/2048) = 0, round(-1000/2048) = 0, round(-2500/2048) = -1
    EXPECT_EQ(pos3.global, int3(0, 0, -1)); 
    EXPECT_NEAR(pos3.local.x, -500.0f, 1e-5f);   // -500 - 0*2048 = -500
    EXPECT_NEAR(pos3.local.y, -1000.0f, 1e-5f);  // -1000 - 0*2048 = -1000
    EXPECT_NEAR(pos3.local.z, -452.0f, 1e-5f);   // -2500 - (-1)*2048 = -452
    
    // Test large coordinates (space sim scale) - just verify it doesn't crash
    double large_coord = 1e10; // 10 billion units
    EXPECT_NO_THROW({
        LargePosition pos4(double3(large_coord, 0.0, 0.0));
        ...
        EXPECT_LE(std::abs(pos4.local.x), LargePosition::CELL_SIZE);
    });
    ...
    LargePosition pos5(double3(offset + 1000.0, offset + 500.0, offset + 1500.0));
    int3 origin = int3(10000, 10000, 10000);
    float3 reconstructed = pos5.to_float3(origin);
    EXPECT_NEAR(reconstructed.x, 1000.0f, 1e-3f);
    EXPECT_NEAR(reconstructed.y, 500.0f, 1e-3f);
    EXPECT_NEAR(reconstructed.z, 1500.0f, 1e-3f);
    
    // Test that equal world coordinates produce equal positions
    LargePosition pos6(double3(2500.0, 0.0, 0.0)); // Should be global=(1,0,0), local=(452,0,0) with center-based cells
    LargePosition pos7(int3(1, 0, 0), float3(452.0f, 0.0f, 0.0f)); // Manual construction
    EXPECT_EQ(pos6, pos7); // Should be equal
} 