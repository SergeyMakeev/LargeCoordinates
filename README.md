...
**Q: In my multiplayer space sim game, 10 players have built a 500-meter-long star destroyer that's traveling far from origin and crossing multiple cell boundaries. How do I handle gameplay simulation to avoid precision errors?**

A: The key insight is that an object's size doesn't determine its cell assignment - only its reference position does. Choose a single reference point for the star destroyer (typically its center of mass or bridge) and store that as a LargePosition. For simulation, convert all player positions and the ship's reference point to a shared coordinate frame using `to_float3()` with the ship's global cell as the reference. This gives you FP32 coordinates in a +/-1024 unit space around the ship where all gameplay logic can run with maximum precision. The ship's internal structure (player positions relative to the ship) should be stored as local offsets from the reference point, so they remain stable regardless of which cells the ship crosses. Update the ship's LargePosition as it moves, and the hysteresis system will handle cell transitions smoothly without affecting internal gameplay.

**Q: How do I simulate a rover moving on the Moon's surface using the LargePosition system?**

A: Set up the Moon as a celestial body with its own LargePosition (e.g., at orbital distance from Earth/Sun). For surface operations, establish a local coordinate system where the rover's LargePosition represents its location on the Moon's surface. You can use a Moon-centered coordinate system where the rover's global coordinates are relative to the Moon's center, and local coordinates represent the precise surface position. When the rover moves across the surface, update its LargePosition normally - the cell system will handle transitions as it traverses different regions of the Moon. For surface-specific calculations (like navigation, obstacle avoidance, or interaction with surface features), convert rover and nearby object positions to a shared reference frame using the rover's current cell. This maintains sub-millimeter precision for detailed surface operations while allowing the rover to traverse the entire Moon without precision loss. The same approach works for any planetary body - Mars, asteroids, space stations, etc.
